...
export default function DailyMetricsCard({ metrics, loading }: Props) {
    ...
    return (
      <div className="bg-surface rounded-xl border border-border p-5">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-medium text-foreground">Métricas de hoje</h3>
          <Link href="/metrics" className="text-xs text-brand-600 hover:underline">Registrar →</Link>
        </div>
        <p className="text-sm text-muted-foreground">Ainda não registradas hoje.</p>
  ...
  return (
    <div className="bg-surface rounded-xl border border-border p-5">
      ...
      {metrics.notes && (
        <p className="text-xs text-muted-foreground mt-3 italic line-clamp-1">"{metrics.notes}"</p>
      )}
    </div>
  );
}
